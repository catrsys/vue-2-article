...
Для того, чтобы увидеть все в это действии, можно посмотреть небольшой проект [TodoMVC](https://codesandbox.io/s/github/vuejs/vuejs.org/tree/master/src/v2/examples/vue-20-todomvc?from-embed), построенный на Firebase API, Vue 2.0 + Vue Router + Vuex, или на  [Клон HackerNews](https://github.com/vuejs/vue-hackernews-2.0), компоненты которого описаны в стиле SFC. Узнать больше о самом Vue 2, особенностях, примерах и способах его использования можно в [официальной документации](https://v2.vuejs.org/v2/guide/).

## Уязвимости Vue 2
Та же [официальная документация Vue 2](https://ru.vuejs.org/v2/guide/security.html) предупреждает разработчиков о том, что не следует использовать ненадёжное содержимое (например, пользовательский ввод) в качестве шаблона компонента:
```JavaScript
new Vue({
  el: '#app',
...
```
Это можно считать основным правилом разработки. К сожалению, его не всегда соблюдают, и поэтому такие примеры превращаются в потенциальные векторы. В той же документации сообщается о том, что при исользовании двойных фигурных скобок (double mustaches) содержимое `userProvidedString` экранируется, как и в динамически привязанных атрибутах:
```JavaScript
...
```
В случаях, когда внедрение доверенного либо санитизированного HTML, CSS и JavaScript необходимо, в документации предлагаются следующие решения:
...
```
Все эти возможности создают определенные риски безопасности, которые могут служить потенциальными уязвимостями и будут рассмотрены далее.
...
```
Как видно на практике, почти все директивы могут быть источниками уязвимостей при отсутствии санитизации. Некоторые атаки, рассмотренные авторами исследования, были разработаны для проектов, в которых Vue используется для рефакторинга кода на JQuery:
```JavaScript
// это не сработает
...
```
Вложенные квантификаторы внутри regexp могли вызвать catastrophic backtracking, и потому [были убраны](https://github.com/vuejs/vue/commit/cd334070f3b82d3f5892c4999cc290ccd4f56fd8), а перед проверкой строк добавлен вызов `trim()`. Уязвимость была актуальна до версии `2.5.17`.
...
На примере библиотек компонентов лишний раз убеждаемся, что необходимо санитизировать пользовательский ввод всегда и везде, ни на кого не надеясь. С другой стороны, при исследовании веб-ресурсов на Vue полезно знать об уязвимостях, вызванных ошибками при разработке и ресурса, и его зависимостей.
...
* заменой вложенных квантификаторов сверхжадными (Posessed Quantifiers), не позволяющими откат внутри себя

**В общем случае** для снижения риска появления уязвимости рекомендуется использовать комплексные меры защиты, не доверять пользовательскому вводу, а также не забывать про `npm aufit` и `npm aufit fix` – команды, которые позволяют разработчику переложить всю головную боль по **отслеживанию уязвимых зависимостей** на пакетный менеджер, а исследователю – быстро и безболезненно получить отчет по уязвимостям клиентской части веб-приложения. Важно отметить, что `npm aufit fix` не просто обновляет зависимости проекта: она заменяет версии,в которых была обнаружена уязвимость, на версии, в которых эта уязвимость либо закрыта, либо еще не появилась.

Весь материал, собранный для Vue 2, актуален (с небольшими адаптациями) и для Vue 3, так как разработчики фреймворков и библиотек не берут на себя ответственность за санитизацию пользовательских данных, о чем [открыто заявляют](https://gist.github.com/pi0/674d49d5f1c2ccfe20d3b1e29ae7b497).
